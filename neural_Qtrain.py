...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    for i, dim in enumerate(HIDDEN_NODES):
        ...
        node_input = layers[i]
        printd('{h} hidden nodes layer assigned {s}: {ni}', h=dim, ni=layers[i], s=layers[i].shape)

    q_values = tf.contrib.layers.fully_connected(layers[-1], action_dim, activation_fn=None)

    printd("q_values {q}", q=q_values)
    q_selected_action = \
        tf.reduce_sum(tf.multiply(q_values, action_in), reduction_indices=1)

    printd("q_selected_values {q}", q=q_selected_action)
    printd("dim {t} x {a}", t=target_in.shape, a=q_selected_action.shape)
