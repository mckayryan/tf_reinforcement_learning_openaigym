...
class DQN(object):
    ...
    def update_t_net(self):
        ...
        printd('\nNew Target Q {tq}\n replaced to {assign}', tq=self.q, assign=self.t_w_assign_op)
    ...
    def define_loss(self, action):
        self.action = action
        printd("dim {t} x {a}", t=self.t_in.shape, a=self.action.shape)
...
def get_action(state, state_in, q_values, epsilon, test_mode, action_dim):
    ...
    Q_estimates = NETWORK.q.eval(feed_dict={s_in: [state]})
...
def get_train_batch(q_values, state_in, replay_buffer):
    ...
    if c.double_q:
        printd('Next Batch shape {s}\ts_in shape {ss}', s=next_state_batch.shape, ss=NETWORK.s_in.shape)
        predictions = np.argmax(NETWORK.q.eval(feed_dict={s_in: next_state_batch}))

        t_s_in = NETWORK.t_s_in
        t_q_idx = NETWORK.t_q_idx

        printd("t_s_in {t}  +  t_q_idx {t_q}", t=t_s_in, t_q=t_q_idx)
        q_value_batch = NETWORK.t_q_w_idx.eval(feed_dict={
            t_s_in: next_state_batch,
            t_q_idx: [[i,p] for i, p in enumerate(predictions)]
        })
        printd("Before\n{b}\n+{c}", b=self.t_q.eval(feed_dict={t_s_in: next_state_batch}, c=t_q_idx))
        printd("After\n{a}", a=q_value_batch.shape)
